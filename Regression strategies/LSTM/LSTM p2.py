# -*- coding: utf-8 -*-
## Importing some relavent packages ##
...
#### This is an LSTM for the SPY prediction at the next point in time ###

### Note throughout the code, it looks messy, some coding lines have been commented out for now.
...
##### We will calculate some technical indicators (extra features) to work with the classical:
##### Open, high, low, close, vol. We are adding Simple moving averages (SMA), exp moving
##### averages (EMA), Relative Strength Index (RSI), Standard dev, and change in price. Each of this are calculated based
##### on a certain number of historical points in time (days represent each point in time in this case).
N=15
...
### index prices, which may allow the LSTM to identify price reversals.
spy_data['PC']=spy_data['Close'].pct_change()
...
### close data column (including the Training, test and val closing values).
target_scaler=MinMaxScaler()
target_scaler.fit(spy_data['Close'].values.reshape(-1, 1))

### When the NN works, it goes off a previous number of days to make a prediction, please trial with this 
### look_back variable. I tried a range, 40 seemed to fit better, but no way good enough.
### Try making a loop testing a range of look back periods.
...
### This function designs the appropriate 3D shape which is given to the NN for training, running an iterative loop
### and a moving window and fitting the model each iteration is not how LSTM has been back-end coded. Needs the whole
### 3D data set inputted as X_train. But I have assumed LSTM casually does mirroring, and that we have to design the 
### output variable dataset appropriately (y_train, y_test, y_val) using spy_data[i+1,0].
...
#### New LSTM model, please edit and find parameters which result in better MSE and MAE.
### Trial different activation functions, numb neurons number of layers, dropout layers....
model = Sequential()
...
#### Inverse scaling of the data is being preformed correctly I believe
